...
#[Route(path: '/position/edit/{position}', name: Edit::ROUTE_NAME, siteAware: true)]
#[IsGranted(UserRole::ROLE_ADMIN->name)]
final class Edit extends AbstractController
{
    public const string ROUTE_NAME = 'position.edit';
    ...
    public function __invoke(Request $request, Site $site, Position $position): array|Response
    {
        $p = clone $position;
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', 'Position edited successfully');

            return $this->redirectToRoute(Lists::ROUTE_NAME, ['site' => $site->getId()->toBase58()]);
        }

        return ['form' => $form->createView(), 'position' => $p];
    }
}
